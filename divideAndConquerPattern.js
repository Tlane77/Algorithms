...
function search(array, val) {
  ...
  while (min <= max) {
    ...
    let middle = Math.floor((min + max) / 2);//floor rounds down and ceiling rounds down(be consistant!)
    ...
    if (array[middle] < val) {
      ...
      return middle;//This gives average(median)
    }
  }
  return -1;//Otherwise return neg 1.
}

console.log(search([1, 2, 3, 6, 9], 9));
console.log(search([3, 6, 9, 12, 15, 17, 25, 30], 12));
